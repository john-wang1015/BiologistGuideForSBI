...
def bvcbm_simulation(p1, p2, p3, m1, m2, m3, eng=None, batch_size=1, random_state=None):
    ...
    for batch_ii in range(batch_size):
        sx = eng.simulator_density_nonparall(theta_matlab, 15,  nargout=1)
...
def run_bsl():
    folder_name = "res/bsl/syn1_density"
    is_exist = os.path.exists(folder_name)
    if not is_exist:
        os.makedirs(folder_name)

    x_obs = sio.loadmat('CellDensity_synthetic_dataset.mat')['sy1']
    true_params  = sio.loadmat('CellDensity_synthetic_dataset.mat')['theta1']
