...
def run_snpe_c():
    x_0 = sio.loadmat('CellDensity_synthetic_dataset.mat')['sy']
    ...
    folder_name = "res/npe/fucci_density"
    ...
    for kk in range(num_rounds):
        ...
        mat_path = os.path.join(folder_name, "snpe_c_nsf fucci density, round {}.mat".format(kk))
        ...
        pic_path = os.path.join(folder_name, 'snpc_fucci_density_iteration {}.png'.format(kk))
    ...
    for idx, model in enumerate(posteriors):
        file_path = os.path.join(folder_name, 'fucci_density_model_{idx}.pth')
