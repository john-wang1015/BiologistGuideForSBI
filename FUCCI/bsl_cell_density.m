function [theta, loglike] = bsl_cell_density(y,init,m,M,cov_rw,prior, file_name)
...
x = zeros(m,length(y));
parfor (k = 1:m,ncpus)
    x(k,:) = simulator_density_nonparall(init, 15, InitPosData, CellTrackingData);
end
...
for i = 1:M
    ...
    parfor (k = 1:m,ncpus)
        ssx(k,:) = simulator_density_nonparall(prop, 15, InitPosData, CellTrackingData);
    end
