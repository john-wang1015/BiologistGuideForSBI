function SMC_ABC_density_syn2
    ...
    %% Observed Data
    CellTracking = false; %TRUE - if using cell trajectory data; FALSE - if using cell density data
    ...
    load('CellDensity_synthetic_dataset.mat','sy2');%[Nred, Nyellow, Ngreen, TotalDistance]; %store observed summary statistics
    sy = sy2;
    ...
    while p_accept > pacc_target && tolmax > tol_target
        ...
        count = count + (N_a + 1)*(MCMC_iters - MCMC_trials - (MCMC_trials + 1));
        ...
        save(sprintf('res/smcabc/density_syn2_progress%d.mat',tt'),'theta_trans', 'p_accept','tolmax','p_unique'); %write performance stats to excel
    end
...
%% save results   
    ...
    filename = sprintf('res/smcabc/results_SMCABC_density_syn2.mat');
