...
def get_model(true_params=None, x_obs=None):
    ...
    elfi.Prior('uniform', 2, 31, model=m, name='tau')
    # simulator
...
def run_rbsl():
    obs = sio.loadmat("CancerDatasets.mat")['Pancreatic_4th_mouse_data'].reshape(32)
    ...
    n_sim = 10000
    true_params = {'g_age': 300.0, 'g_age2': 200.0, 'tau': 9.0}
    ...
    est_posterior_cov = sio.loadmat("cov_matrix_pan4.mat")['cov_matrix']
    ...
    res.plot_traces()
    plt.savefig('bsl_bvcbm_traces_pan4i.png')


    mbins = 30
    res.plot_marginals(reference_value=true_params,
                       bins=mbins)
    plt.savefig('bsl_bvcbm_marginals_pan4i.png')

    res.plot_pairs(reference_value=true_params,
                   bins=mbins)
    plt.savefig('bsl_bvcbm_pairs_pan4i.png')

    folder_name = "res/bsl/pan4i"
    ...
    plt.savefig('bsl_bvcbm_gamma_pan4i.png')
