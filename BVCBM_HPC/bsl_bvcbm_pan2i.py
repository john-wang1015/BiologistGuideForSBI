...
def get_model(true_params=None, x_obs=None):
    ...
    m = elfi.ElfiModel()
    # prior
    elfi.Prior('uniform', 2, (26*24)-3, model=m, name='g_age')
    elfi.Prior('uniform', 2, (26*24)-3, model=m, name='g_age2')
    elfi.Prior('uniform', 2, 25, model=m, name='tau')
    # simulator
...
def run_rbsl():
    ...
    obs = pancreatic[0:26,1]
    m = get_model(x_obs = obs)
    likelihood = pdf_methods.standard_likelihood("variance")
    feature_names = ['log_S']
    seed = 1
    ...
    n_sim = 10000
    true_params = {'g_age': 500.0, 'g_age2': 200.0, 'tau': 9.0}
    ...
    est_posterior_cov = sio.loadmat("cov_matrix_pan2.mat")['cov_matrix']
    est_posterior_cov = np.array(est_posterior_cov).reshape((3,3))
    logit_transform_bound = [(2, (26*24)-1),
                             (2, (26*24)-1),
                             (2, 25)]
    
    elfi.set_client('multiprocessing')

    params0 = [500.0, 200.0, 9.0]
    ...
    res.plot_traces()
    plt.savefig('bsl_bvcbm_traces_pan2i.png')


    mbins = 30
    res.plot_marginals(reference_value=true_params,
                       bins=mbins)
    plt.savefig('bsl_bvcbm_marginals_pan2i.png')

    res.plot_pairs(reference_value=true_params,
                   bins=mbins)
    plt.savefig('bsl_bvcbm_pairs_pan2i.png')

    folder_name = "res/bsl/pan2i"
    is_exist = os.path.exists(folder_name)
    if not is_exist:
        os.makedirs(folder_name)
    with open(f'{folder_name}bsl_thetasi.pkl', 'wb') as f:
    ...
    plt.savefig('bsl_bvcbm_gamma_pan2i.png')
