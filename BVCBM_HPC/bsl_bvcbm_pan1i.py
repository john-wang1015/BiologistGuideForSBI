...
def run_rbsl():
    ...
    res.plot_traces()
    plt.savefig('bsl_bvcbm_traces_pan1i.png')

    mbins = 30
    res.plot_marginals(reference_value=true_params,
                       bins=mbins)
    plt.savefig('bsl_bvcbm_marginals_pan1i.png')

    res.plot_pairs(reference_value=true_params,
                   bins=mbins)
    plt.savefig('bsl_bvcbm_pairs_pan1i.png')
    ...
    #plt.savefig('bsl_bvcbm_gamma_pan1.png')

    folder_name = "res/bsl/pan1i"
    ...
    gamma_dict = dict(zip(['gamma_{}'.format(index) for index in range(r_bsl_v.observed.size)],
                          np.transpose(res.outputs['gamma'])))
    elfi.visualization.visualization.plot_marginals(gamma_dict, bins=mbins)
    plt.savefig('bsl_bvcbm_gamma_pan1i.png')
