...
def run_snpe_c():
    x_0 = sio.loadmat("paper2_BVCBM_synthetic_datasets.mat")['sim1']

    prior_min = torch.as_tensor([0,0,17,2.0, 2.0,0,0,17, 2.0])
    prior_max = torch.as_tensor([1, 1e-4,18, 19*24-1.0, 18.0,1, 1e-4,18, 19*24-1.0])
    ...
    for kk in range(num_rounds):
        ...
        plt.savefig('snpc_bvcbm_syn1_iteration {}.png'.format(kk))
        plt.show()
        
        string2save = "snpe_c_nsf syn1, round {}.mat".format(kk)
