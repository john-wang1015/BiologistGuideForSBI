...
def get_model(true_params=None, x_obs=None):
    if true_params is None:
        true_params = np.array([100.0, 300.0,  16.0])
...
def run_rbsl():
    obs = sio.loadmat("paper2_BVCBM_synthetic_datasets.mat")['sim3']
    ...
    n_sim = 10000
    true_params = {'g_age': 100.0, 'g_age2': 300.0, 'tau': 16.0}
    ...
    est_posterior_cov = sio.loadmat("cov_matrix_syn3.mat")['subset_matrix']
    ...
    res.plot_traces()
    plt.savefig('bslv_bvcbm_traces_syn3i.png')


    mbins = 30
    res.plot_marginals(reference_value=true_params,
                       bins=mbins)
    plt.savefig('bslv_bvcbm_marginals_syn3i.png')

    res.plot_pairs(reference_value=true_params,
                   bins=mbins)
    plt.savefig('bslv_bvcbm_pairs_syn3i.png')
    ...
    plt.savefig('bslv_bvcbm_gamma_syn3i.png')

    folder_name = "res/bsl/syn3i"
