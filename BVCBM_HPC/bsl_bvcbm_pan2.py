...
def run_rbsl():
    ...
    res.plot_traces()
    plt.savefig('bsl_bvcbm_traces_pan2.png')


    mbins = 30
    res.plot_marginals(reference_value=true_params,
                       bins=mbins)
    plt.savefig('bsl_bvcbm_marginals_pan2.png')

    res.plot_pairs(reference_value=true_params,
                   bins=mbins)
    plt.savefig('bsl_bvcbm_pairs_pan2.png')

    folder_name = "res/bsl/pan2"
    is_exist = os.path.exists(folder_name)
    if not is_exist:
        os.makedirs(folder_name)
    with open(f'{folder_name}bsl_thetas.pkl', 'wb') as f:
    ...
    plt.savefig('bsl_bvcbm_gamma_pan2.png')
