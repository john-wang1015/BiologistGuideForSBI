...
def bvcbm_simulation(theta):
    ...
    sx_all = np.zeros((len(theta.tolist()), 26))
    print(sx_all.shape)
    sx_all = eng.simulator_pal(theta_matlab, 26,  nargout=1)
    eng.quit()
...
def run_snpe_c():
    ...
    x_0 = pancreatic[0:26,1]

    prior_min = torch.as_tensor([0,0,17,2.0, 2.0,0,0,17, 2.0])
    prior_max = torch.as_tensor([1, 1e-4,18, 26*24-1.0, 25.0,1, 1e-4,18, 26*24-1.0])
    ...
    for kk in range(num_rounds):
        ...
        plt.savefig('snpc_bvcbm_pan2_maf_iteration {}.png'.format(kk))
        plt.show()
        
        string2save = "snpe_c_maf pan2, round {}.mat".format(kk)
