...
def get_model(true_params=None, x_obs=None):
    if true_params is None:
        true_params = np.array([200.0, 75.0,  16.0])
...
def run_rbsl():
    obs = sio.loadmat("paper2_BVCBM_synthetic_datasets.mat")['sim2']
    ...
    n_sim = 10000
    true_params = {'g_age': 200.0, 'g_age2':75.0, 'tau': 16.0}
    ...
    est_posterior_cov = sio.loadmat("cov_matrix_syn2.mat")['subset_matrix']
    ...
    res.plot_traces()
    plt.savefig('bslv_bvcbm_traces_syn2i.png')


    mbins = 30
    res.plot_marginals(reference_value=true_params,
                       bins=mbins)
    plt.savefig('bslv_bvcbm_marginals_syn2i.png')

    res.plot_pairs(reference_value=true_params,
                   bins=mbins)
    plt.savefig('bslv_bvcbm_pairs_syn2i.png')
    ...
    plt.savefig('bslv_bvcbm_gamma_syn2i.png')

    folder_name = "res/bsl/syn2i"
