...
def get_model(true_params=None, x_obs=None):
    if true_params is None:
        true_params = np.array([300.0, 200.0,  9.0])
...
def run_rbsl():
    ...
    res.plot_traces()
    plt.savefig('rbsl_bvcbm_traces_pan4.png')


    mbins = 30
    res.plot_marginals(reference_value=true_params,
                       bins=mbins)
    plt.savefig('rbsl_bvcbm_marginals_pan4.png')

    res.plot_pairs(reference_value=true_params,
                   bins=mbins)
    plt.savefig('rbsl_bvcbm_pairs_pan4.png')

    folder_name = "res/rbsl/pan4"
    ...
    plt.savefig('rbsl_bvcbm_gamma_pan4.png')
